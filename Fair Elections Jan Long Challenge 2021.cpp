Task: Elections are coming soon. This year, two candidates passed to the final stage. One candidate is John Jackson and his opponent is Jack Johnson.
...
space-separated integers B1,B2,…,BM

    .

Output

For each test case, print a single line containing one integer ― the smallest number of swaps needed to make John Jackson win, or −1
...
Example case 1: We can perform two swaps ― each time, we swap a pack of 2
votes from A and a pack of 5 votes from B. After that, John Jackson gets 5+5=10 votes and Jack Johnson gets 2+2+5=9

votes.

Example case 2: We can swap the pack of 1
vote from A and the pack of 8 votes from B. After that, John Jackson gets 8+3+2+4=17 votes and Jack Johnson gets 6+7+1=14 votes.
...
int main(){
	...
	cin>>t;
	while(t--){
		int n,k=0,a,m,sumn=0,cnt=0,summ=0,temp,sumn1,summ1;
		vector<int> v,d;
		cin>>n>>m;
		for(int i=0;i<n;i++){
			cin>>a;
		...
		if(sumn>summ){
			...
			while(k<v.size()&&k<d.size()){
				temp = d[k];
				d[k]=v[k];
				v[k]=temp;
				cnt++;
				k++;
				sumn1=accumulate(v.begin(),v.end(),0);
				summ1=accumulate(d.begin(),d.end(),0);
				if(sumn1>summ1){
					cout<<cnt<<endl;
					break;
				}
